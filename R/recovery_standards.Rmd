...
for (i in 1:length(filenames[,1])) {
  name = paste0("~/Work/Messdaten/Selina_Daten/eval_varied_ppm/", filenames[i,1])
  result = processPeak(name, standardMasses)
  result_table = rbind(result_table, result)
  result_table$file[i+1] = filenames[i,1] # first row is empty due to initialization
}
...
standardsTable = function(filename, standards, ppm){
  ...
  matchingFeatures = data.frame(std_mass = -1, foundMZ = -1, foundRT = -1, 
                                fountInt = -1, otherMatches = -1)
  tmpData = read.csv(filename)
  for (i in 1:n) {
    select = tmpData[which(tmpData$mz > (standards[i] - ppm * tmpData$mz)),]
    select = select[which(select$mz > (standards[i] - ppm * select$mz)),]
...
elimination = function(matchGroup, toleranceRT){ # only works if not-founds are not included
  ...
  for (i in 1:length(matchGroup$mz)) {
    ...
    if(eliminated$found[namePos]) {
      groupBreaks = cbind(groupBreaks, i)
  ...
  groupBreaks = cbind(groupBreaks, length(matchGroup$mz))
