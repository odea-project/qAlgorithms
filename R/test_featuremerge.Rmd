...
curve_0_M = function(x, scale_to_A, feature_A, feature_B){ # merged curve with apex at x = rt and height = 1
  ...
  b2_m = (feature_A$b2 + feature_B$b2) / 2
  b3_m = (feature_A$b3 + feature_B$b3) / 2
  # if no intersect exists, move the non-apex curve so t
  # maxp = -b1_n1 / (2 * b2_m)
  # b0_n1 = b0_n1 - (b0_n1 + b1_n1 * maxp + b2_m * maxp * maxp) 
  root = sqrt((b1_n1 - b1_n2)^2 - 4* (b2_m - b3_m) * (b0_n1 - b0_n2))
  if(is.nan(root)) {
  ...
  # root = sqrt((b1_n1 - b1_n2)^2 - 4* (b2_m - b3_m) * (b0_n1 - b0_n2))
  # if(is.nan(root)) {return(0)} # invalid set of coefficients
  # offset = (b1_n2 - b1_n1 ) / (2 * (b2_m - b3_m)) + root / (2 * (b2_m - b3_m))
  offset = (feature_A$retentionTime + feature_B$retentionTime) / 2
...
```{r}
## different approach: form the mean and perform a regression using the existing model
# case 1: do not shift

# case 2: shift to apex = 0
