...
s_Kali__H2O_4_pos %>% group_by(errorcode) %>% summarise(bin_count = n(), perc_dataset = round(sum(size) / allpoints * 100, digits = 2), ratio = round(sum(size)/n(), digits = 2), dqsCen = mean(DQSC_min), meanDQS = mean(DQSB_scaled)) 
