        ...
        qPeaks::passToBinning(std::vector<std::vector<q::DataType::CentroidPeak>> &allPeaks, std::vector<unsigned int> addEmpty)
        {
            ...
            for (size_t i = 0; i < allPeaks.size(); ++i)
            {
                ...
                if (!allPeaks[i].empty())
                {
                    ...
                    std::sort(allPeaks[i].begin(), allPeaks[i].end(), [](const q::DataType::CentroidPeak a, const q::DataType::CentroidPeak b)
