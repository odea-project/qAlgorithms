...
namespace qAlgorithms
{
    ...
    const std::array<float, 384> INV_ARRAY = initialize();   // @todo make constexpr
    ...
    bool massTraceStable(std::vector<float> massesBin, int idxStart, int idxEnd) // do this in regression
    {
    ...
    for (std::filesystem::path pathSource : tasklist)
    {
        ...
        for (auto polarity : polarities)
        {
            ...
            std::vector<EIC> binnedData = performQbinning(
                ...
                !userArgs.verboseProgress, userArgs.printSummary, userArgs.printBins);
