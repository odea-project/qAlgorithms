...
namespace q
{
      ...
      std::vector<int> &sorted_indexes)
  {
    ...
    const double diff = std::pow(data[1][index + 1] / data[1][index], 1.0 / (gapSize + 1));
    for (int j = 1; j <= gapSize; j++)
    {
      data[0].push_back(data[0][index] + j * expectedDifference); // add interpolated x value
      data[1].push_back(data[1][index] * std::pow(diff, j));      // add interpolated y value
      for (size_t l = 2; l < numRows; l++)
      {
        data[l].push_back(0.0); // add zeros to all other rows in data
      }
      sorted_indexes.push_back(sorted_indexes[index] + j); // add sorted indices for the interpolation
    }
  }
  ...
  {
    // calculate the interpolated y-axis values using log interpolation
    const double diff = std::pow(data[1][index + gapSize + 1] / data[1][index], 1.0 / (gapSize + 1));
    for (int j = 1; j <= gapSize; j++)
    {
      data[1][index + j] = data[1][index] * std::pow(diff, j); // interpolated y value
      data[2][index + j] = 0.0;                                // add zeros to all other rows in data
    }
  }
      ...
      std::vector<std::vector<double>> &data)
  {
    // calculate the interpolated y-axis values using log interpolation
    const double diff = std::pow(data[1][index + gapSize + 1] / data[1][index], 1.0 / (gapSize + 1));
    const int numZeroesPerSide = static_cast<int>((gapSize - numAddedZeroes) * .5);
    // check if gapSize - numAddedZeroes is even
    if (!((gapSize - numAddedZeroes) % 2 == 0))
    {
      std::cerr << "Error: gapSize - numAddedZeroes must be even" << std::endl;
    }
    // interpolate left side
    for (int j = 1; j <= numZeroesPerSide; j++)
    {
      data[1][index + j] = data[1][index] * std::pow(diff, j); // interpolated y value
      data[2][index + j] = 0.0;                                // add zeros to all other rows in data
    }
    // interpolate right side
    for (int j = 0; j < numZeroesPerSide; j++)
    {
      data[1][index + 2 * numAddedZeroes - j] = data[1][index] * std::pow(diff, gapSize - j); // interpolated y value
      data[2][index + 2 * numAddedZeroes - j] = 0.0;                                          // add zeros to all other rows in data
    }
    // interpolate middle
    for (int j = 0; j < numAddedZeroes; j++)
    {
      data[1].end()[-j] = data[1][index] * std::pow(diff, gapSize - numZeroesPerSide - j); // interpolated y value
    }
  }
  ...
  extrapolate_x_left(
      ...
      std::vector<int> &sorted_indexes)
  {
    for (int j = -separatingZeros; j < 0; j++)
    {
      data[0].push_back(data[0][index + 1] + j * expectedDifference);
      for (size_t l = 1; l < numRows; l++)
      {
        data[l].push_back(0.0);
      }
      sorted_indexes.push_back(sorted_indexes[index] + 1 + (2 * separatingZeros + j));
    }
  }
  ...
  extrapolate_x_right(
      ...
      std::vector<int> &sorted_indexes)
  {
    for (int j = 1; j <= separatingZeros; j++)
    {
      data[0].push_back(data[0][index] + j * expectedDifference);
      for (size_t l = 1; l < numRows; l++)
      {
        data[l].push_back(0.0);
      }
      sorted_indexes.push_back(sorted_indexes[index] + j);
    }
  }
