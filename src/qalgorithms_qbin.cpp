...
#pragma region "Rawdata"
        UnbinnedData::UnbinnedData() {}
        UnbinnedData::UnbinnedData(std::vector<std::vector<std::unique_ptr<q::DataType::Peak>>> &data)
        {
            int now = time(0);
            lengthAllPoints = 0;
            allDatapoints.push_back(std::vector<Datapoint>(0)); // first element empty since scans are 1-indexed
            // @todo COMPLETE THIS FUNCTION - convert data to Datapoints
            
        }
        q::Algorithms::UnbinnedData::~UnbinnedData() {} // potential memory leak
        bool UnbinnedData::readcsv(std::string user_file, int d_mz, int d_mzError, int d_RT, int d_scanNo, int d_intensity, int d_DQScentroid)
...
#pragma omp parallel for
            ...
            return true;
            // UnbinnedData is always a vector of vectors where the first index is the scan number (starting at 1) and every scsn is sorted by mz
        }
...
#pragma region "BinContainer"
        ...
        void BinContainer::makeFirstBin(UnbinnedData *rawdata)
        {
        ...
        void BinContainer::assignDQSB(const UnbinnedData *rawdata, const unsigned int maxdist)
        {
            auto timeStart = std::chrono::high_resolution_clock::now();
#pragma omp parallel for
        ...
        void BinContainer::printAllBins(std::string path, const UnbinnedData *rawdata)
        {
...
#pragma region "Bin"
        ...
        Bin::Bin(UnbinnedData *rawdata) // @todo relatively time intensive, better solution?; why does rawdata need to be mutable?
        {
        ...
        void Bin::makeDQSB(const UnbinnedData *rawdata, const unsigned int maxdist)
        {
...
int main()
{
    ...
    q::Algorithms::UnbinnedData testdata;
    ...
    q::Algorithms::BinContainer testcontainer;
    testcontainer.makeFirstBin(&testdata);
    std::vector<int> measurementDimensions = {q::Algorithms::SubsetMethods::mz, q::Algorithms::SubsetMethods::scans}; // at least one element must be terminator
    ...
    std::cout << "Total duplicates: " << q::Algorithms::duplicatesTotal << "\n--\ncalculating DQSBs...\n";

    // calculate data quality scores
    q::Algorithms::scaleDistancesForDQS_gauss(inputMaxdist); // this sets q::scalarForMOD to the correct scaling so that at the distance in scans == maxdist the curve encloses 99% of its area
