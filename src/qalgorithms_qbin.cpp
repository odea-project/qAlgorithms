...
#pragma region "Bin"
    ...
    void Bin::makeCumError()
    {
        ...
        std::transform(pointsInBin.begin(), pointsInBin.end(), back_inserter(cumError), [](qCentroid *F)
                       { return F->mzError; });                               // *1000000 for compatibility with ppm
...
#pragma endregion "Functions"
                                     ...
                                     std::string filename, int maxdist, bool silent, bool printBinSummary, bool printCentroids)
    {
        ...
        // this sets scalarForMOD to the correct scaling so that at the
...
//     qAlgorithms::duplicatesTotal = 0;

//     int inputMaxdist = 6;
//     qAlgorithms::maxdist = inputMaxdist;
...
//     qAlgorithms::CentroidedData testdata;
...
//     if (!qAlgorithms::readcsv(&testdata, filename_input, 0, 1, 3, 4, 6)) // ../../rawdata/control_bins.csv reduced_DQSdiff
//     // if (!readcsv(&testdata, filename_input, 1, 2, 3, 4, 5))
//     {
//         exit(101); // error codes: 1.. = reading / writing failed, 2.. = improper input,
//     }

//     qAlgorithms::BinContainer testcontainer;
//     testcontainer.makeFirstBin(&testdata);
//     std::vector<int> measurementDimensions = {qAlgorithms::SubsetMethods::mz, qAlgorithms::SubsetMethods::scans}; // at least one element must be terminator
...
//     std::cout << "Total duplicates: " << qAlgorithms::duplicatesTotal << "\n--\ncalculating DQSBs...\n";

//     // calculate data quality scores
//     qAlgorithms::scaleDistancesForDQS_linear(inputMaxdist); // this sets scalarForMOD to the correct scaling so that at the distance in scans == maxdist the curve encloses 99% of its area
//     testcontainer.assignDQSB(&testdata, inputMaxdist, false);           // int = max dist in scans

//     testcontainer.redoBinningIfTooclose(measurementDimensions, &testdata, qAlgorithms::notInBins, qAlgorithms::maxdist);
