...
#pragma region "print functions"
                        ...
                        std::string filename, bool silent, bool skipError, bool noOverwrite)
    {
        ...
        output << "cenID,mz,mzUncertainty,scanNumber,retentionTime,area,areaUncertainty,"
               << "height,heightUncertainty,scale,degreesOfFreedom,DQSC,interpolations,extrapolations,competitors\n";
        ...
        for (size_t i = 0; i < peaktable.size(); i++)
        {
            if (!peaktable[i].empty())
            {
                for (size_t j = 0; j < peaktable[i].size(); ++j)
                {
                    ...
                    char buffer[256];
                    sprintf(buffer, "%d,%0.6f,%0.6f,%d,%0.4f,%0.4f,%0.4f,%0.4f,%0.4f,%d,%u,%0.5f,%d,%d,%d\n",
                            counter, peak.mz, peak.mzUncertainty, peak.scanNumber, convertRT[peak.scanNumber],
                            peak.area, peak.areaUncertainty, peak.height, peak.heightUncertainty, peak.scale, peak.df, peak.dqsCen,
                            peak.interpolations, peak.extrapolations, peak.numCompetitors);
                    output << buffer;
                    ++counter;
                }
            }
                          ...
                          bool verbose, bool silent, bool skipError, bool noOverwrite)
    {
        ...
        output << "ID,binID,binIdxStart,binIdxEnd,mz,mzUncertainty,retentionTime,retentionTimeUncertainty,"
               ...
               << "scale,interpolations,competitors,DQSC,DQSB,DQSF,apexLeft,b0,b1,b2,b3\n";
        ...
        for (size_t i = 0; i < peaktable.size(); i++)
        {
            ...
            char buffer[256];
            sprintf(buffer, "%d,%d,%d,%d,%0.6f,%0.6f,%0.4f,%0.4f,%0.4f,%0.4f,%0.3f,%0.3f,%0.3f,%0.3f,%d,%d,%d,%0.5f,%0.5f,%0.5f,%s,%0.8f,%0.8f,%0.8f,%0.8f\n",
                    ...
                    peak.interpolationCount, peak.competitorCount, peak.dqsCen, peak.dqsBin, peak.DQSF,
