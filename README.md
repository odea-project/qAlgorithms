...
* Binning will generate a lot of EICs that do not contain any chromatographic peak. It is
  expected behaviour that only every fifth to sixth bin contains one or more peaks.
...
parameters. Binning allows you to group centroids into extracted ion chromatograms
based purely on the mass dimension and time dimension. The current `qBinning` program is based
