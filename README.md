...
and is very fast / computationally efficient (see [Design Philosophy](#design-philosophy) for more).
...
from R, we are collaborating with the [patRoon project by Helmus et al](https://github.com/rickhelmus/patRoon).
...
To build from source, install [mingw](https://www.msys2.org/). After installation, run the command
```sh
...
only test for gcc, but clang should also work), gnu make, cmake and the zlib-ng headers. The 
script `setup.sh` provided in the main directory will check missing dependencies and
...
Note that we link explicitly against zlib-ng, even if zlib on the target platform is already
linking it in compatibility mode.
...
* Check out the [documentation](./docs/) for more details on using `qAlgorithms`.
...
provide additional layers of data. For the user, it is thulsy possible to treat
the output features as the commonly found mz -- RT -- intensity triplet or to
consider more precise descriptors. These are summarised in the data quality score,
a number between zero and one, that is roughly equivalent to the commonly used R².
For in-depth details, some statistics regarding the processing routines are
preserved as part of the peak output.
...
The project uses C++, but further employs extensive compiler warnings and static analysis
tools to ensure program correctness. 
...
The mathematical description of the peak model is available in detail [in the documentation](./docs/moments_peakmodel.md).

The algorithm as it is implemented includes a set of decision rules for selecting
the best fit and modifications to the design matrix that were not part of the 
initial publication and are as of now undocumented.
...
used to determine the critical value for the used test is described [in the documentation](./docs/explanation_critval_qbinning.md).
...
* [zlib](https://www.zlib.net/), for decompression of mzML binary data (gnu-mingw under windows).
* [zlib-ng](https://github.com/zlib-ng/zlib-ng), for decompression on linux platforms.
...
one of these issues, you are welcome to open a pull request for one of these items.
