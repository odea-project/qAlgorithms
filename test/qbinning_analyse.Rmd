...
```{r scanbreaks}
scanbreaks <- read.csv("../../qBinning_scanbreaks.csv")
scanbreaks$size = c(diff(scanbreaks$position),3568034-3566759)
```

```{r bin results}
qbins <- read.csv("../../qBinning_binlist.csv")
subbin <- qbins[which(qbins$bin == 4871),]

plot(subbin$mz, subbin$scan)

subbin[order(subbin$mz),]

nos <- diff(subbin$mz)
3.05037165842070 * (log(787 + 1)^(-0.4771864667153))
```

```{r steps per bin}
getwd()
vec <- as.vector(read.csv("../../qBinning_stepsForDQS.csv"))
```
