...
for (i in 1:length(monobin$mz)) {
  t1 = c(monobin$mz[i]*runif(1, 2, 3), monobin$error[i], runif(1, 700, 2000), monobin$scans[i], 1, -1, 0, -1)
  t2 = c(monobin$mz[i]*runif(1, 2, 4), monobin$error[i], runif(1, 700, 2000), monobin$scans[i], 1, -1, 0, -1)
...
plot(qbins$control_DQSB, qbins$DQS)
