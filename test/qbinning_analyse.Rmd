...
# summary <- read.csv("G:/_Studium/Analytik-Praktikum/qbinning/rawdata/warburg_summary.csv")
summary$errorcode = as.numeric(lapply(summary$errorcode, bits))
# bins <- read.csv("G:/_Studium/Analytik-Praktikum/qbinning/rawdata/warburg_selectBins_full.csv") 
...
summary %>% group_by(errorcode) %>% summarise(bin_count = n(), total_points = sum(size), ratio = sum(size)/n(), dqsCen = mean(DQSC_min), meanDQS = mean(DQSB_scaled)) 
...
binViewer = function(summary, fullbins, binID){
  ...
  while (rateNext) {
    ...
    if(input == "#"){
      ...
      print(ggplot(fullbins[which(fullbins$ID == binID),])+
      geom_point(aes(scan, intensity, colour = mz), size = 2)+
      geom_col(aes(scan, intensity, fill = mz), colour = "lightblue1", width = 0.3)+
