...
ggplot(summary_error[summary_error$errorcode - round(summary_error$errorcode, -5) >= 1000,])+
...
ggplot(summary_error[summary_error$errorcode - round(summary_error$errorcode, -3) >= 100,])+
  geom_point(data = notbinned[subsample_notbinned,], aes(x = mz, y = scan)) +
  geom_point(aes(x = mean_mz, y = mean_scans, colour = as.factor(errorcode)))

summary_error %>% group_by(errorcode) %>% summarise(bin_count = n(), total_points = sum(size), ratio = sum(size)/n(), dqsCen = mean(DQSC_min))
...
binRater = function(summary, fullbins, startNo){
  cat("Rate bins using numbers:
        0: The bin is good as-is (one region of high intensity)
        1: The bin is good, but was assigned an error
        2: There is a possible peak and noise data present
        3: One or more possible peaks are connected by noise data
        4: There is no peak visible here - error found
        5: The wrong error was assigned
        6: No peak visible - no error
        7: mass drift with intensity / peak shape
        8: few outliers seem possible
...
ggplot(bins_error[which(bins_error$ID == 4210),])+
  geom_point(aes(mz, scan, color = intensity))+ # , color = as.factor(ID)
  xlab("m/z")+
  ylab("Scan Number")+
  theme(panel.background = element_rect(fill = "lightblue1",
  ...
  scale_color_viridis_c(option = "plasma", direction = -1)+
  labs(caption = sprintf("Errorcode: %d", summary_error$errorcode[1]))
