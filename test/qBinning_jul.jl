...
mutable struct qBin
    idx::Vector{Int64}
    os::Vector{Float64}
    mos::Vector{Float64} ## maximum of os, this is only set if there was a split at this point
    critV::Float64
end
...
function createIndex(x::Vector{Vector{Float64}},critVal_lib::qBinCrit)::Dictionary{Int64,qBin}
    ...
    insert!(idx, 1, qBin(collect(1:nTotal),x[2],zeros(nTotal),crit))
    return(idx)
end
    ...
    for (t,u) in zip(keys(idxDict),idxDict)
        if u.critV > 0
            ...
            if (maximum(u.os) / err) > u.critV
                updateCutIdx!(u,idxDict,minBinSize,critVal_lib,model_nos, err)
                flag = true
            else
    ...
    if (cutIdx >= minBinSize) | ((N-cutIdx) >= minBinSize)
        if cutIdx >= minBinSize
            if (N-cutIdx) >= minBinSize
                model_nos == :gaussian ? insert!(idxDict, lastindex(idxDict)+1, qBin(bin.idx[cutIdx+1:end],bin.os[cutIdx+1:end],bin.mos[cutIdx+1:end],critVal_lib.val[N-cutIdx])) : nothing
                model_nos == :discrete ? insert!(idxDict, lastindex(idxDict)+1, qBin(bin.idx[cutIdx+1:end],bin.os[cutIdx+1:end],bin.mos[cutIdx+1:end],critVal_lib.val[end])) : nothing
            end
            for u in [:idx,:os, :mos]
                deleteat!(getproperty(bin,u), collect(cutIdx+1:N))
            end
            bin.mos[end] = bin.os[end] / err
            bin.os[end] = -255.
...
function loadData(df::DataFrame)
    ...
    nos = append!(diff(DF[!, "mz"]),-255.)#./mean(df[!, "Centroid Error"])
...
function qBinning(df::DataFrame) #::Tuple{DataFrame, Dictionary{Int64, qBin}}
    ...
    return DF, idxDict, nnd, nndw, mz
end

### RUN
# if @isdefined(df)
    import_file ="C:\\Users\\grenn\\Desktop\\control_bins.csv"
    ...
    sort!(df, :mz)
# end
DF, idxDict, nnd, nndw, mz = qBinning(df)
"complete"

# gDF = groupby(DF, :ID)
# gaps = [maximum(diff(sort(u[!,"Scans"]))) for u in gDF]
# maximum(gaps)
# sizes = [length(u[!,"Scans"]) for u in gDF]
# minimum(sizes)
# # find mz = 102.943140969428 in DF
# findall(DF[!,"mz"] .== 102.943140969428)
# DF[17383,:]
# bin = findall(DF[!, "ID"] .== 240)
# DF[bin,:mz]

# masses = [findall(df[!,"mz"] .== u)[1] for u in DF[bin,:mz]]
# df[masses,:]
# mean(df[masses,:error])
# maximum(diff(sort(df[masses,:mz])))
# cvalues[16]
# ## create csv file of mz with the bin
# row = string("mz\n")
# open("C:\\Users\\grenn\\Desktop\\bins.csv", "a") do io
#     write(io, row)
# end
...
# ### debugging, csv:
# # create a csv file with df[idxDict[i].idx[end],:mz],idxDict[i].mos[end] iterating over idxDict
# row = string("mz_first,mzlast,mos,length\n")
# open("C:\\Users\\grenn\\Desktop\\cuts.csv", "a") do io
#     write(io, row)
# end

# for u in idxDict
#     # create string for a row:
#     row = string(df[u.idx[1],:mz], ",", df[u.idx[end],:mz],",",u.mos[end], ",", length(u.idx),"\n")
#     # append row to file
#     open("C:\\Users\\grenn\\Desktop\\cuts.csv", "a") do io
#         write(io, row)
#     end
