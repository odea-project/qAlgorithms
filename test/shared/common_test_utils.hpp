...
inline void default_assertion_handler(const char *assertion_as_cstring, const char *file, int line, const char *format_string, ...)
{
    ...
    (void)fprintf(stdout, message_string, file, line, assertion_as_cstring);
    (void)vfprintf(stdout, format_string, argument_list);
...
inline int64_t nanoseconds(void)
{
    struct timespec p[1];
...
double randRange_d(double lower, double upper, int64_t seed = 0);
