...
```{r summary-summary function}
sumsum = function(summaryFrame){
  lowDQSpercentage = length(summaryFrame$ID[which(summaryFrame$DQSB_scaled < 0.2)]) / length(summaryFrame$ID)
  summaryFrame = summaryFrame[which(summaryFrame$DQSB_scaled < 0.2),]
...
sum_all$testview = (sum_all$mean_DQS - sum_all$groupDQS) * log(sum_all$bin_count) * sum_all$total_points / sum_all$errorfac
