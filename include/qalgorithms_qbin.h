...
#pragma region "Bin"
    ...
    public:
        ...
        /// @param rawdata the std::vector<qCentroid> object from which the bin was generated
        /// @param maxdist the largest gap in scans which a bin can have while still being considered valid
        // void makeDQSB(const std::vector<qCentroid> *rawdata, const std::vector<float> scalarForMOD, const size_t maxdist);
    ...
    bool binLimitsOK(Bin sourceBin, const std::vector<qCentroid> *rawdata, const int maxdist);
...
#pragma endregion "Bin Container"
    ...
    /// @brief wrapper function to execute qbinning on a std::vector<qCentroid> struct
