...
namespace q
{
    namespace Algorithms
    {
        ...
        class UnbinnedData //@todo rename to centroided data?
        {
        public:
            UnbinnedData(); // @todo add handling for reading in data without centroid error
            UnbinnedData(std::vector<std::vector<std::unique_ptr<q::DataType::Peak>>> &data);
            ~UnbinnedData();
            int lengthAllPoints;
        ...
        public:
            ...
            /// @param rawdata the UnbinnedData object from which the bin was generated
            /// @param maxdist the largest gap in scans which a bin can have while still being considered valid
            void makeDQSB(const UnbinnedData *rawdata, const unsigned int maxdist);
        ...
        public:
            ...
            /// @param rawdata a set of raw data on which binning is to be performed
            void makeFirstBin(UnbinnedData *rawdata); // @todo find way for rawdata to be const
            ...
            /// @param rawdata the UnbinnedData object from which all bins in finishedBins were generated
            /// @param maxdist the largest gap in scans which a bin can have while still being considered valid
            void assignDQSB(const UnbinnedData *rawdata, const unsigned int maxdist);

            void printAllBins(std::string path, const UnbinnedData *rawdata); // @todo remove rawdata dependency
        ...
        void check_MOD_outOfBins(const Bin *target, const std::vector<q::Algorithms::Datapoint *> notBinned, const int maxdist);
...
#endif // _QALGORITHMS_QBIN_INCLUDED